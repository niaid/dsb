...
In all cases encountered thus far, very extreme value ranges (e.g. ranging from -50 to 30) are caused by just a few outlier cells, most often a few cells with low negative values for the protein. For very high values after dsb normalization (~100) check how many cells are in this quantile-these can be removed as outliers (or retained but removed for visualization purposes). We have now provided a quantile clipping option in dsb to address these outlier cells. Users can also investigate these cells to see if they have very high values for isotype control proteins and they can possibly be removed from the dataset. 
...
abs(cor(d$cellwise_background_mean, d$dsb_technical_component))
