...
We also filter background droplets to remove potential spurious cells (drops with high mRNA contene) and use the major peak in the background distribution between 1.4 and 2.5 log total protein counts. 
...
# umap (this is a long step and in our opinion can often be skipped)
...
NEW: dsb users keen on using python are also encouraged to checkout the dsb wraper available in the `muon` python multimodal framework 
...
more info: https://github.com/niaid/dsb/issues/10

**the range of dsb normalized values is large is this normal?**
In all cases encountered thusfar, very extreme value ranges (e.g. ranging from -50 to 30) are caused by just a few outlier cells, most often a few cells with low negative values for the protein. For very high values after dsb normalization (~100) check how many cells are in this quantile-these can be removed as outliers (or retained but removed for visualization purposes). 
https://github.com/niaid/dsb/issues/22
https://github.com/niaid/dsb/issues/9
 
```{r, eval = FALSE}
# find outliers 
pheatmap::pheatmap(apply(dsb_norm_prot, 1, function(x){quantile(x,c(0.9999, 0.99, 0.98, 0.95, 0.0001, 0.01, 0.1) )}))

```


Very low negative values, e.g. those < -10 can be interpreted as not expressed and one can simply convert these values to 0 with the code below. 
```{r, eval = FALSE}
dsb_norm_prot = apply(dsb_norm_prot, 2, function(x){ ifelse(test = x < -10, yes = 0, no = x)})
```
...
**How can I generate a count histogram and marginal density histogram of positive and negative droplet protein distributions like the one in the paper?**
