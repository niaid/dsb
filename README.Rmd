...
We next visualize the log library size (total reads) of mRNA and protein. We coloring cells by MT gene proportion to identify potentially lysed / low quality cells (which have high MT gene content) and the location of the greatest density (the greatest number) of droplets. The leftmost plot shows the location of the major background distribution (the yellow area of high density / many empty droplets with ADT reads). We can see in the 3rd plot from the left with empty droplets colored by MT gene proportion that some may be low-quality apoptotic cells. We next set upper and lower cutoffs on the background matrix library size. The Normalized values are robust to different background thresholds used, so long as one does not omit this major background population (See Supplementary Fig. 7 in the paper for more information). 
...
We are now ready to use dsb to normalize and denoise the ADTs. For data with isotype control proteins, set `denoise.counts = TRUE` and `use.isotype.control = TRUE` and provide a vector containing names of isotype control proteins (the rownames of the protein matrix that are isotype controls). For data without isotype controls, see the additional vignettes on CRAN.
```{r, eval = FALSE}
...
This tutorial is a guide. It cam be modified to fit your needs. For topics see these vignettes on CRAN:

### understanding dsb
a "under the hood" guide with every assumption and modeling step to get more intuition for how the method works. 
