...
Only 5,000 cells are expected to be recovered from this experiment = these represent a tiny fraction of the total droplets- the peak around 2 is the major background distribution and the cells are are in the population barely visible in this histogram > 3.


The library size distribution of cells and empty droplets after QC (these steps were used above to define negative an pisitve droplets shown for reference) are shown below with the number of each population shown = this gives a sense of the umber of barcodes represtignig droplete with ambient protein , > 50,000 vs cells ~ 3800 in line with what we expect from the experiment loading and QC. 
```{r, eval = FALSE}
neg_drops2 = md %>%
  rownames_to_column("bc") %>% 
  filter(log10umiprot < 2.5 & log10umiprot > 1.4)  %>% 
